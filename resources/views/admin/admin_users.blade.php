        ...
        <div class="rounded-lg pb-4">
            <div class="container-fluid p-0 z-50 flex flex-wrap justify-center w-full h-auto">
                ...
                <div class="w-full flex flex-col lg:flex-row justify-between p-2 border-b-2 border-gray-200 gap-4">
                    ...
                    <div class="mx-3">
                        <form action="#" class="flex flex-wrap gap-3">
                            <div class="w-full sm:w-auto">
                                <div class="relative">
                                    <select
                                        class="bg-white placeholder:text-slate-400 text-gray-500 text-sm border border-slate-500 rounded h-10 pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 appearance-none cursor-pointer w-full sm:w-auto">
                                        <option value="brazil">Select Category</option>
                                        <option value="bucharest">Admin</option>
                                        <option value="london">Normal</option>
                                    </select>
                                </div>
                            </div>
                            <div class="w-full sm:w-auto">
                                <div class="w-full max-w-sm min-w-[200px] relative">
                                    <div class="relative">
                                        <input
                                            class="bg-white w-full pr-11 h-10 pl-3 py-2 placeholder:text-slate-400 text-slate-700 text-sm border border-slate-500 rounded transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400"
                    ...
                    <button onclick="window.location.href='{{ url('add_user') }}'"
                    ...
                    <table class="w-full text-left table-auto min-w-max">
                        <thead>
                            <tr class="border-b border-slate-300">
                                <th class="p-4 text-sm font-normal leading-none text-slate-500">Images</th>
                                ...
                                <th class="p-4 text-sm font-normal leading-none text-slate-500">Role</th>
                                ...
                                <tr class="hover:bg-slate-50">
                                    <td class="p-4 border-b border-slate-500 py-5">
                                        <img src="{{ $user->profile_image ? asset('images/users/' . $user->profile_image) : asset('images/users/main_user_image.webp') }}"
                                            alt="User 1" class="w-16 h-16 object-cover rounded" />
                                    </td>
                                    <td class="p-4 border-b border-slate-500 py-5">
                                    ...
                                    <td class="p-4 border-b border-slate-500 py-5">
                                        <button onclick="window.location.href='{{ url('view_user') }}/{{ $user->id }}'"
                                            class="font-medium text-green-600 hover:underline cursor-pointer">View</button>
                                        <button
                                            onclick="window.location.href='{{ url('edit_user') }}/{{ $user->id }}'"
                                            class="font-medium text-blue-600 hover:underline cursor-pointer ms-3">Edit</button>
                                        <button
                                            onclick="window.location.href='{{ url('remove_user') }}/{{ $user->id }}'"
